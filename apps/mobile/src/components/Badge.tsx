...
import { formatUrgency, formatBloodGroup } from '../utils/format';

// Both badges accept raw backend codes (CRITICAL, O_POS) or display labels
// (Urgent, O+) and always render the human label.
export const UrgencyBadge: React.FC<{ level: UrgencyLevel | string }> = ({ level }) => {
  const label = formatUrgency(level);
  const style = urgencyColors[label as UrgencyLevel] ?? urgencyColors.Medium;
  return (
...
export const BloodGroupBadge: React.FC<{ group: BloodGroup | string; size?: 'sm' | 'md' | 'lg' }> = ({ group, size = 'md' }) => {
  const label = formatBloodGroup(group);
  const bg = bloodGroupColors[label as BloodGroup] ?? colors.primary;
  ...
  return (
    <View style={[styles.bloodBadge, { backgroundColor: bg, width: dim, height: dim, borderRadius: dim / 2 }]}>
      <Text style={[styles.bloodText, { fontSize: fs }]}>{label}</Text>
    </View>
  );
};
